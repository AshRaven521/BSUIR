...
def differences_method(start_variables_count,
    ...
    d[0] = d[0] - a_k[0] * y_a
    d[-1] = d[-1] - c[-1] * y_b
    answer = lab2.sweep_method(a_k, b_k, c, d)
...
def task_1():
    ...
    fig, _ = plt.subplots()
    plt.plot(D, func_y5, c='red', label='нб 5')
    plt.plot(D, func_y6, c='green', label='нб 6')
    plt.plot(D, func_y7, c='blue', label='нб 7')
...
def task_3():
    def specified(a, b, t_0, t_n, h, tetta, fi, g1, g2, k, f, func_for_partition):
        ...
        for index_t, time in enumerate((j for j in np.arange(t_0, t_n, tetta)), 1):
            ...
            solving[index_t] = lab2.sweep_method(a_k, b_k, c_k, d_k)
        return solving
