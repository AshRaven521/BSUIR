...
{
	top--;
	sum.erase(sum.end() - 1);/*end возвращает итератор на место после последнего элемента вектора,поэтому добавляем -1
...
double stackdecode::Calc(const std::string& pnote)
{
   ...
   /*	if(pch==","&&pch==""&&pch=="."&&pch=="!"&&pch=="/")
	{
		MessageBox(,L"Введены лишние символы",L"Сообщение",NULL);
	}*/
	while(pch!=NULL)
	{
		if(*pch=='+')
