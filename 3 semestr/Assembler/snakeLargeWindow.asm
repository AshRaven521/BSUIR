...
;Очищаем игровое поле(устанавливаем видеорежим AH=00,AL=03(80×25 стандартный 16-цветный текстовый режим))
	...
	xor bx,bx	
	mov ax,0B800h;0b800h соответствует сегменту дисплея в текстовом режиме(сегментный адрес текстового буфера)
	mov es,ax
...
;Рисуется верхняя строка(зелене элементы)
	mov cx,50h
up_:	
...
;Рисуются средние строки(красные элементы)
	mov cx,17h;количество строк
draw:
	add bx,0h ;рисуем левый борт (количество пробелов до вывода борта)
	mov word ptr es:[bx],dx	
...
;Рисуется начальный счет
	add bx,0h	
	mov dl,Score
	add dl,'0'	
	mov word ptr es:[bx],dx;Операция word ptr указывает ассемблеру, что данный операнд в памяти нужно интерпретировать как слово
	add bx,2
	mov word ptr es:[bx],dx
	add bx,2
	mov dl,0B2h
...
back:
	;Выводим счет
	mov ax,0200h
	mov dx,1801h;Устанавливаем номер строки 18h и позиция в строке 1h
	int 10h	
	mov ah,02h
	mov dl,Score
	add dl,'0'
...
next_score:
	...
	;Выводим счет
	mov ax,0200h
	mov dx,1800h;Устанавливаем номер строки 18h и позиция в строке 0h
	int 10h 
