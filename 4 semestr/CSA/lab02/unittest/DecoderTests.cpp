...
TEST_SUITE("Decoder"){
    ...
    TEST_CASE("INDIVIDUAL_TEST")
    {
        Word instructionWord = 0b00000000111101111010011000100011;
        
        auto decoded = _decoder.Decode(instructionWord);

        CHECK(decoded->_imm.value() == IMM_S);
        CHECK(decoded->_src2.value() == 15);
        CHECK(decoded->_src1.value() == 15);
        CHECK(decoded->_type == IType::St);
    }
}
