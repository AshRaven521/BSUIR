...
def lagrange_interpolation(_data_, x=0.1):
    ...
    for i in range(len(_data_)):
        result += _data_[1][i] * (
            weight_coefficient(data_src=_data_, x=x, exclude=i) / weight_coefficient(data_src=_data_, x=data[0][i], exclude=i))
    return result
...
def newton_interpolation(_data_, x=0.1):
    def _a_(_data=None, top_idx=None):
        ...
        for k in range(top_idx + 1):
            result += _data_[1][k] / weight_coefficient(data_src=_data_, x=_data[0][k], exclude=k, top_value=top_idx + 1)
        return result
