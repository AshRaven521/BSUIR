...
A = np.add(np.multiply(C, option), D)


def jacobi(A, b, init_vector=None):
    B = np.eye(A.shape[0]) - A / np.diag(A).reshape(-1, 1)
    if min(np.linalg.norm(B, ord=norm)
           for norm in (np.inf, 1, 'fro')) < 1:
...
def compare_answers(func, arg=None):
    ...
    print(
        f"{'True answer':<45} : {_true_x},\n{func.__name__ + ' answer':<45}:" + f"{(func(A, b) if arg is None else func(A, b, arg)).reshape(-1)}")
